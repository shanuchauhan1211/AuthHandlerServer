...
// Serve frontend build (adjust for Vite/Next if needed)
app.use('/', express.static(path.join(__dirname, 'frontend-build')));

// Proxy /api to backend on Device B
app.use('/api', createProxyMiddleware({
  target: 'http://192.168.5.29:8081', // backend device IP
  changeOrigin: true,
  secure: false,
  pathRewrite: {
    '^/api': '', // optional: remove `/api` prefix if backend doesn't expect it
  }
}));
